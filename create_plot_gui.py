...
def fits(image):
    ...
    image = cv2.resize(
        image, (width, int(image.shape[0] / scale_factor)), interpolation=cv2.INTER_AREA
    )
...
class MainApplication(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.filemenu.add_command(
            label="Simulationsergebnis laden", command=self.load_file
        )
    ...
    def load_file(self):
        ...
        self.filename = fd.askopenfilename(
            title="Datei auswählen...", initialdir=os.getcwd(), filetypes=self.filetypes
        )
        if self.filename == "":
            return
        self.show_preview()
    ...
    def show_preview(self):
        ...
        self.photoimage = ImageTk.PhotoImage(self.image, master=self.canvas)
